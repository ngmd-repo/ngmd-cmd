...
function convertToJSONFormat(fileContent, locale) {
  ...
  const errorMessage = `Locale "${locale}" doesn't exist. Please use available locales: ${Object.keys(
    translateMap
  )}`;
...
function createTranslateJSONFile(path, filename, jsonTranslateMap) {
  ...
  writeFileSync(
    `${path}/${filename}`,
    JSON.stringify(jsonTranslateMap, null, 2)
  );
}
...
export function useLocaliseStrategy(commandArgs) {
  ...
  const jsonTranslateMap = convertToJSONFormat(
    fileContent,
    args["upload-locale"]
  );

  createTranslateJSONFile(
    args["output-path"],
